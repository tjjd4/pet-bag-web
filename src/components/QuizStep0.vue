...
const rules: Record<keyof FormData, any> = {
  petType: { required },
  petAge: { required },
  petGender: { required }
};

const questions: IQuestion<FormData>[] = [
  { label: "您的愛寵是？", model: 'petType', options: [
    { text: "貓咪", value: "cat" },
    { text: "狗狗", value: "dog" }
  ]},
  { label: "寵物年齡是幾歲？", model: 'petAge', options: [
    { text: "0-2 歲 (幼年)", value: "age_0_2" },
    { text: "3-5 歲 (成年)", value: "age_3_5" },
    { text: "5-10 歲 (中年)", value: "age_5_10" },
    { text: "10+ 歲 (老年)", value: "age_10_plus" }
  ]},
  { label: "寵物的生理性別？", model: 'petGender', options: [
    { text: "男", value: "male" },
    { text: "女", value: "female" }
  ]}
];
...
<template>
  <div v-for="question in questions" :key="question.label">
    <RadioGroup :label="question.label" :options="question.options" v-model="form[question.model as keyof FormData]" class="my-5" />
